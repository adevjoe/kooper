...
func (p *Preparer) SetUp() {
	ns, err := p.cli.CoreV1().Namespaces().Create(context.TODO(), p.namespace, metav1.CreateOptions{})
...
func (p *Preparer) TearDown() {
	err := p.cli.CoreV1().Namespaces().Delete(context.TODO(), p.namespace.Name, metav1.DeleteOptions{})
