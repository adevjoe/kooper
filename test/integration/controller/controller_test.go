...
import (
	...
	"github.com/adevjoe/kooper/v2/test/integration/helper/cli"
	"github.com/adevjoe/kooper/v2/test/integration/helper/prepare"
)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, svc := range test.addServices {
				_, err := k8scli.CoreV1().Services(prep.Namespace().Name).Create(context.TODO(), svc, metav1.CreateOptions{})
				assert.NoError(err)
				time.Sleep(1 * time.Second)
			}

			for _, svc := range test.updateServices {
				origSvc, err := k8scli.CoreV1().Services(prep.Namespace().Name).Get(context.TODO(), svc, metav1.GetOptions{})
				if assert.NoError(err) {
					// Change something
					origSvc.Spec.Ports = append(origSvc.Spec.Ports, corev1.ServicePort{Name: "updateport", Port: 9876})
					_, err := k8scli.CoreV1().Services(prep.Namespace().Name).Update(context.TODO(), origSvc, metav1.UpdateOptions{})
					assert.NoError(err)
					time.Sleep(1 * time.Second)
				}
			}
